...
"""
HBM3 命令调度性能模型——v21.1 中文候选版
================================================================

版本信息
--------
版本          : v21.1
发布状态      : 新增 Col Lock Window 特性 (跨 read_cam / write_cam 统一跟踪活跃 burst).
              详细说明见下面 "v21.1 关键特性" 区块.
              默认 col_lock_window_enable=False, 行为完全等同 v20.1.
              + 继承 v20.1 / v20.0 / v19.3 / v19.2 全部特性
注释语言      : 中文
行为兼容性    : 默认 batch_scheduling=True + rw_4state_mode=True 走 RW 4 态机 (v17+ 推荐);
              设 batch_scheduling=True + rw_4state_mode=False 走原 batch/preparation (向后兼容);
              设 batch_scheduling=False 强制 Alternating, 此时 rw_4state_mode 被忽略.
              默认 write_requires_data_ready=True 与 v19.3 一致;
              设 False 可恢复 v19.1 baseline (无 buffer 模型).
              默认 col_lock_window_enable=False 与 v20.1 一致;
              设 True 启用跨 CAM 统一窗口跟踪活跃 burst.
...
v21.1 关键特性 (本期新增, 重点)
---------------------------------
1. **新增 Col Lock Window 特性** (跨 read_cam / write_cam 统一窗口跟踪活跃 burst)
     - 目的: 限制同时活跃的 burst 数 (默认 size=4), 模拟 CAM 资源约束场景,
       防止某些 burst 在 CAM 中长期滞留. 当 burst 的 first col 派发时占一个 slot,
       该 burst 全部 col 派发完才释放 slot (与 CAM remove 同步发生).
     - 准入 (新 burst 想 first col): bank 不与现有 entry 重叠 + window 未满.
     - 软优先 (多候选都满足准入): BG 与窗口内 entry 不同者优先, 否则任意.
     - 默认 `col_lock_window_enable=False`, 行为完全等同 v20.1 (无窗口概念).
     - 用法: `HBMCommandScheduler(..., col_lock_window_enable=True, col_lock_window_size=4)`
     - **架构**: 配置**只在 HBMCommandScheduler 暴露** (唯一默认来源).
       ColScheduler 通过 `enable_col_lock_window(size)` 方法接收启用指令,
       不暴露独立默认值, 避免双默认值不一致 bug.
2. **Col Lock Window 准入与 BG 软优先集成**:
     - `ColScheduler._try_issue_of_type()` 在原有 dispatchable 收集后,
       拆分 in_window (已激活 burst 的 cmd) / out_window (新候选 cmd).
     - `in_window` 优先于 `can_admit` (BG 软优先仅对 can_admit 生效).
     - `_can_admit_to_window()` 内累加 full_count / bank_conflict_count 统计.
3. **`_complete_dispatch` 维护 window 状态**:
     - first col 派发完成 → `_admit_to_window(burst_entry)` (占一个 slot).
     - last col 派发完成 → `_release_from_window(burst_entry)` (与 CAM remove 同步).
4. **新增 `get_col_lock_stats()` 与 summary 区块**: 暴露窗口使用情况统计.
5. **校验**: size < 1 (enable=True) 时报错, 统一在 HBMCommandScheduler 入口校验.
6. **兼容性**: 默认 col_lock_window_enable=False 时 perf 与 v20.1 完全一致;
   启用后 (True) 行为变化符合设计预期 (random workload +1.5% 到 +4.4% 提升,
   linear workload 受 BG 软优先约束轻微下降).
7. **新增 Bank 打开时长统计** (ACT 至 bank 真正 IDLE 的 cycle 数)
     - 目的: 度量每次 bank "打开→关闭" 周期的平均时长, 反映 bank 利用率
       与打开成本 (tRAS + col 派发时间 + tRP). 高 → bank 长期占用; 低 → 频繁开关.
     - 采样: bank 从 IDLE→ACT_WAIT 起点, 到 PRE_WAIT/AUTO_PRE_WAIT/REFRESHING
       完成回到 IDLE 的终点 (含 tRP 完成), 记 1 个样本.
     - 聚合: 整个测试期所有 bank 的样本合在一起, 输出 count/avg/min/max/p50/p95/sum.
     - 多次 IDLE 转换路径 (force-pre, 显式 PRE, WRA/RDA, REFRESHING) 都正确处理,
       REFRESHING→IDLE 不重复计入 (PRE_WAIT→IDLE 已先结算).
     - 仿真结束仍处打开周期的 bank (未完成样本) 不计入, 避免偏差.
     - summary 区块 "[Bank 打开时长统计 (v21+, ACT 至 bank 真正 IDLE 的 cycle 数)]".
     - 用法: 无需配置, 自动收集, 始终输出. 不影响 perf.


v20.1 关键特性 (上版本, 保留作为历史)
---------------------------------
1. **`batch_scheduling` 提升为大类开关**
     - 目的: 明确 R/W 调度模式的层级关系, 防止 `rw_4state_mode` 与 `batch_scheduling`
       的耦合导致旧 batch 路径在某些参数组合下走错分支.
     - 新层级: `batch_scheduling` 决定大类 (alternating vs batch),
       `rw_4state_mode` 仅在 `batch_scheduling=True` 时有效 (4 态机 vs 原 batch).
     - 行为差异:
       * batch_scheduling=False  → 强制 Alternating, `rw_4state_mode` 被忽略.
       * batch_scheduling=True + rw_4state_mode=True   → RW 4 态机 (默认).
       * batch_scheduling=True + rw_4state_mode=False  → 原 batch/preparation (向后兼容).
2. **修正调度分支拼写 & 路径顺序**
     - `ColScheduler.try_issue()` 的判断顺序由原先的 `rw_4state_mode` 优先
       调整为 `batch_scheduling` 优先, 并修复 `_batch_sscheduling` 拼写错误;
       解决 `rw_4state_mode=False` 且 `batch_scheduling=True` 时因 NameError
       跌入 Alternating 的隐性回归.
3. **`get_row_candidate_cams()` 调整 CAM 可见性顺序**
     - 顺序改为: batch_scheduling → rw_4state_mode → batch 准备期/正常态.
     - 效果: `batch_scheduling=False` 时 RowScheduler 始终同时看到 READ/WRITE
       CAM, 不再因 rw_4state_mode=True 被错误收缩到单 CAM, 与 v19 之前一致.
4. **修复 `_check_preparation_exit()` 判空调用**
     - 旧 v20.0 在准备期 `current_empty` 分支调用 `_cam_has_unprocessed(cam)` 时
       缺少必填 `current_cycle` 参数, 在 `batch_scheduling=True + rw_4state_mode=False`
       路径下直接抛 TypeError.
     - 修正: 调用点改为 `_cam_has_unprocessed(cam, current_cycle)`,
       顺带过滤尚未 data-ready 的 WRITE, 与 buffer 语义对齐
       (write_requires_data_ready=True 时, 未 commit 的 entry 不再误判为可调度).
5. **兼容性**: 默认参数组合 (`batch_scheduling=True + rw_4state_mode=True`) 与 v20.0
   完全一致; 仅当用户显式修改上述开关才会触发新行为; 旧参数组合下的 perf 偏差
   (如 rw_4state_mode=False 旧版崩溃) 在 v20.1 已修复, 默认 workload 表现不变.

v20.0 关键特性 (上版本, 保留作为历史)
---------------------------------
...
v19.2 : 新增 `write_data_buffer_depth` (默认 8192) 与 `write_data_ready_delay` (默认 0);
        引入 `data_ready_cycle` 字段与统一的 `_is_entry_data_ready` 入口;
        WRITE admit/commit/释放机制与 12+ 处调度过滤统一基于 data ready.
v19.3 : 修复 `_complete_dispatch` 中 write_data_buffer over-release 逻辑 (commit+release 净 0);
        默认 `write_data_buffer_depth` 由 128 调整为 8192 以避免默认 workload 受约束;
        撤回 WRITE admit 端 buffer 约束 (与 HBM 真实流程不符), 改由 FIFO head-block 承担.
v20.0 : 新增 `write_requires_data_ready` 参数 (默认 True, 保持 v19.3 行为);
        False 模式跳过 `_commit_pending_write_data` 与 buffer 释放, 恢复 v19.1 baseline;
        _validate_inputs / WRITE admit / _complete_dispatch 适配.
v20.1 : 修复 R/W 调度模式开关层级 (`batch_scheduling` 提升为大类, Alternating 强制 CAM 可见性);
        修正 `try_issue` / `get_row_candidate_cams` 调用顺序与 `_batch_sscheduling` 拼写;
        修复 `_check_preparation_exit` 未向 `_cam_has_unprocessed` 传递 `current_cycle`
        导致的必填参数异常, 并在 data ready 语义下正确判空;
        默认参数组合 perf 与 v20.0 完全一致.
"""
...
def _format_col_lock_stats(stats: dict) -> List[str]:
    """v21+: 把 ColScheduler.get_col_lock_stats() 格式化成 summary 行.

    字段:
      enabled             — Col Lock Window 是否启用
      size                — 窗口容量
      used                — 仿真结束时窗口当前已用 slot 数 (通常为 0, 若 >0 则有未完 burst)
      peak_used           — 仿真期内窗口最大同时占用 slot 数 (观察是否触及 size 上限)
      full_count          — 因 window 满导致候选 burst 被阻塞的累计次数
      bank_conflict_count — 因 bank 冲突导致候选 burst 被阻塞的累计次数
      bg_priority_hits    — 选优时实际命中 BG-different 优先级的次数
      total_admits        — 总 admit 次数 (≈ total_releases, 仿真结束时窗口应清空)
      total_releases      — 总 release 次数
    """
    if not stats['enabled']:
        return ["    Col Lock Window     : 禁用 (等同 v20.1, 无窗口机制)"]
    return [
        f"    enabled           : True (跨 R/W CAM 统一跟踪活跃 burst)",
        f"    window 容量       : {stats['size']}",
        f"    仿真末 used       : {stats['used']}  "
        f"(通常 0; >0 表示有未派发完毕的 burst)",
        f"    peak_used         : {stats['peak_used']} / {stats['size']}  "
        f"(窗口峰值占用, 高 = 经常接近满)",
        f"    full 阻塞次数     : {stats['full_count']}  "
        f"(window 满时新 burst 的 first col 被卡)",
        f"    bank 冲突阻塞次数 : {stats['bank_conflict_count']}  "
        f"(新 burst 的 bank 与现有 entry 重叠被卡)",
        f"    BG 软优先命中次数 : {stats['bg_priority_hits']}  "
        f"(选优时 BG-different 候选被优先选中)",
        f"    total admits       : {stats['total_admits']}",
...
def _format_bank_open_duration_stats(stats: dict) -> List[str]:
    ...
    return [
        ...
        f"    平均时长         : {stats['avg']:.2f} cycles",
        f"    最小时长         : {stats['min']} cycles",
        f"    最大时长         : {stats['max']} cycles",
        f"    中位 P50         : {stats['p50']} cycles",
        f"    高分位 P95       : {stats['p95']} cycles",
        f"    累计打开时长     : {stats['sum']} cycles  "
...
class SimulationConfig:
    ...
    # ---- Col Lock Window 配置 ----
    col_lock_window_enable: bool       #: True 启用跨 CAM 统一窗口跟踪活跃 burst; 关闭等同 v20.1
    col_lock_window_size: int          #: 窗口容量 (最多同时锁定的 burst 数); enable=True 时生效
...
class DRAMBank:
    ...
    def __init__(self, bank_id: int, bank_group_id: int):
        ...
        # v21+: bank 打开时长统计 (从 ACT 到 bank 真正回 IDLE 的 cycle 数)
        self._open_start_cycle: int = INITIAL_CYCLE_SENTINEL  # 当前打开周期起点
        self._open_in_progress: bool = False                  # bank 是否处于打开周期
        self._open_durations: List[int] = []                  # 已完成打开周期时长 (cycles)
        ...
        if self.state == BankState.ACT_WAIT:
            ...
            if current_cycle >= self.last_pre_at + timing.t_rp_cycles:
                if self._force_pre_for_refresh:
                    ...
                    self._record_open_close(current_cycle)
                else:
                    self._reset_to_idle()
    ...
    def _record_open_close(self, current_cycle: int) -> None:
        """v21+: bank 真正回到 IDLE 时, 记录本次打开周期时长 (cycles).

        起点: _open_start_cycle (在 RowScheduler._try_activate 派发 ACT 时设置).
        终点: current_cycle (bank 真正变 IDLE 的 cycle, 含 tRP/tRFCpb 完成).
        ...
        if self._open_in_progress:
            duration = current_cycle - self._open_start_cycle
            if duration > 0:
...
class RowScheduler:
    ...
    def _try_activate(self, current_cycle: int,
        ...
        bank._open_start_cycle = current_cycle
...
class ColScheduler:
    ...
    def __init__(self,
        ...
        注: Col Lock Window 配置不在此暴露 (HBMCommandScheduler 是唯一配置入口).
        构造后窗口默认 disable, 行为等同 v20.1. 如需启用, 由 HBMCommandScheduler
        通过 ``enable_col_lock_window(size)`` 显式调用.
        """
        self._banks = banks
        self._timing = timing
        ...
        # ---- Col Lock Window (跨 read_cam / write_cam 统一跟踪活跃 burst) ----
        # 默认 disable (行为等同 v20.1). 启用方法: enable_col_lock_window(size)
        # 由 HBMCommandScheduler 在构造后根据用户参数显式调用.
        # 语义: burst 的 first col 派发时占一个 slot, 该 burst 所有 col 派发完后释放.
        # 准入 (新 burst 想 first col):  bank 不与现有 entry 重叠 + window 未满.
        # 软优先 (多候选都满足准入):    BG 与窗口内 entry 不同者优先, 否则任意.
        self._col_lock_window_enable: bool = False
        self._col_lock_window_size: int = 0
        self._col_lock_window: List[Optional['BurstCommandGroup']] = []
        self._col_lock_used: int = 0
        self._col_lock_peak_used: int = 0
        self._col_lock_full_count: int = 0          # window 满导致新 burst 被阻塞的次数
        self._col_lock_bank_conflict_count: int = 0  # bank 冲突导致新 burst 被阻塞的次数
        self._col_lock_bg_priority_hits: int = 0    # 选优时实际命中 BG-different 的次数
        self._col_lock_total_admits: int = 0        # 总 admit 次数
        self._col_lock_total_releases: int = 0      # 总 release 次数
    ...
    def _try_issue_of_type(self, current_cycle: int,
        ...
        选择流程 (v21+):
          ...
          2. (enable 时) Col Lock Window 准入过滤:
             - 已在 window 中的 burst 优先 (续发)
             - 不在 window 中的 burst: bank 不冲突 + window 未满 → 准入
             - 不准入的 burst 跳过 (统计 full_count / bank_conflict_count)
          3. (enable 时) 多个新 burst 都准入 → BG 与窗口内 entry 不同者优先
          4. SID-aware 选优 (跨 SID 软优化, 与 Col Lock Window 正交)
          5. RR 选 bank_id 命中
        """
        ...
        # ---- Col Lock Window 准入 + BG 软优先 (仅 enable 时生效) ----
        if self._col_lock_window_enable:
            in_window = [(b, c) for b, c in dispatchable
                         if self._is_in_window(b)]
            out_window = [(b, c) for b, c in dispatchable
                          if not self._is_in_window(b)]

            # 过滤 out_window: 准入检查
            can_admit = []
            for burst, cmd in out_window:
                if self._can_admit_to_window(burst):
                    can_admit.append((burst, cmd))
                # _can_admit_to_window 内部已累加 full / bank_conflict 统计

            if in_window:
                # 优先已激活 burst 续发 (与 BG 优先级正交)
                pool = in_window
            elif can_admit:
                # 新 burst 想进入 window: BG 软优先
                in_window_bgs = {e.bank_group_id
                                 for e in self._col_lock_window if e is not None}
                bg_diff = [(b, c) for b, c in can_admit
                           if b.bank_group_id not in in_window_bgs]
                if bg_diff:
                    pool = bg_diff
                    self._col_lock_bg_priority_hits += 1
                else:
                    pool = can_admit
            else:
                # 窗口内不可发 + 窗口外都不可入 → 本 cycle 此 type 无 col 可派
                return None
        else:
            pool = dispatchable
    ...
    def _complete_dispatch(self, burst_entry: BurstCommandGroup,
        ...
        # ---- Col Lock Window: 维护 burst 在 window 中的生命周期 ----
        # first col 派发完成 → 占一个 slot (锁定剩余 col 直至派发完毕)
        # last col 派发完成 → 释放 slot (与 CAM remove 同步发生)
        ...
        if self._col_lock_window_enable:
            if was_first_dispatch:
                # 第一个 col 派发完成 → 进入 window
                # 注: page-hit chain 场景下 next_page_hit 是另一 burst, 它的 admit
                #     在下一个 cycle 它的 first col 派发时才发生, 这里不动.
                self._admit_to_window(burst_entry)
            if will_be_done:
                # 最后一个 col 派发完成 → 离开 window (与下面 CAM remove 同步)
                self._release_from_window(burst_entry)
    ...
    # ---- Col Lock Window 辅助方法 ----

    def enable_col_lock_window(self, size: int) -> None:
        """启用 Col Lock Window (公开 API, 由 HBMCommandScheduler 调用).

        调用后, window 立即生效, 后续 col 派发按 v21+ 的准入 + BG 软优先规则选优.
        通常在仿真开始前调用一次. 已 enable 后再次调用等价于 reset.

        参数:
            size: 窗口容量 (>= 1), 即最多同时跟踪的活跃 burst 数.

        异常:
            ValueError: size < 1
        """
        if size < 1:
            raise ValueError(f"col_lock_window_size 必须 >= 1, 当前 {size}")
        self._col_lock_window_enable = True
        self._col_lock_window_size = size
        self._col_lock_window = [None] * size
        self._col_lock_used = 0
        self._col_lock_peak_used = 0
        self._col_lock_full_count = 0
        self._col_lock_bank_conflict_count = 0
        self._col_lock_bg_priority_hits = 0
        self._col_lock_total_admits = 0
        self._col_lock_total_releases = 0

    def _is_in_window(self, burst_entry: 'BurstCommandGroup') -> bool:
        """检查 burst_entry 是否已在 Col Lock Window 中 (identity 比较).

        disable 时永远返回 False (window 为空, 没有任何 in-window entry).
        """
        if not self._col_lock_window_enable:
            return False
        return any(e is burst_entry for e in self._col_lock_window)

    def _admit_to_window(self, burst_entry: 'BurstCommandGroup') -> None:
        """将 burst_entry 占入 Col Lock Window 一个空闲 slot.

        调用前提: 调度模块已确认 (window 有空 slot + bank 不冲突).
        """
        if not self._col_lock_window_enable:
            return
        for i, slot_ref in enumerate(self._col_lock_window):
            if slot_ref is None:
                self._col_lock_window[i] = burst_entry
                self._col_lock_used += 1
                if self._col_lock_used > self._col_lock_peak_used:
                    self._col_lock_peak_used = self._col_lock_used
                self._col_lock_total_admits += 1
                return
        # 理论上不应到达: 调用方已通过 _can_admit_to_window 检查
        raise RuntimeError(
            "Col Lock Window 已满但试图 admit — "
            "上层 _try_issue_of_type 准入检查漏判"
        )

    def _release_from_window(self, burst_entry: 'BurstCommandGroup') -> None:
        """从 Col Lock Window 释放 burst_entry 的 slot.

        调用前提: burst_entry 已在 window 中 (即 burst 的 first col 已派发过).
        """
        if not self._col_lock_window_enable:
            return
        for i, slot_ref in enumerate(self._col_lock_window):
            if slot_ref is burst_entry:
                self._col_lock_window[i] = None
                self._col_lock_used -= 1
                self._col_lock_total_releases += 1
                return
        # 理论上不应到达: burst 在 _complete_dispatch 被调用说明已经 first col 派发过
        raise RuntimeError(
            "burst 不在 Col Lock Window 中但被要求 release — "
            "admit/release 配对异常"
        )

    def _can_admit_to_window(self, burst_entry: 'BurstCommandGroup') -> bool:
        """检查 burst_entry 是否可以准入 Col Lock Window (占一个 slot).

        准入条件:
          1. window 未满 (_col_lock_used < _col_lock_window_size)
          2. burst 的 bank 不与现有任何 entry 的 bank 重叠

        disable 时永远返回 True (无窗口概念, 所有候选都可入).
        副作用: 失败时累加 _col_lock_full_count / _col_lock_bank_conflict_count 统计.
        """
        if not self._col_lock_window_enable:
            return True
        # 已在 window 中的 burst (例如 page-hit chain) 不需要重新准入
        if self._is_in_window(burst_entry):
            return True
        if self._col_lock_used >= self._col_lock_window_size:
            self._col_lock_full_count += 1
            return False
        in_window_banks = {e.bank_id for e in self._col_lock_window if e is not None}
        if burst_entry.bank_id in in_window_banks:
            self._col_lock_bank_conflict_count += 1
            return False
        return True

    def get_col_lock_stats(self) -> dict:
        """返回 Col Lock Window 统计 (供 summary 输出).

        disable 时 enabled=False, 其他字段归零 (避免污染).
        """
        if not self._col_lock_window_enable:
            return {
                "enabled": False,
        ...
        return {
            "enabled": True,
            "size": self._col_lock_window_size,
            "used": self._col_lock_used,
            "peak_used": self._col_lock_peak_used,
            "full_count": self._col_lock_full_count,
            "bank_conflict_count": self._col_lock_bank_conflict_count,
            "bg_priority_hits": self._col_lock_bg_priority_hits,
            "total_admits": self._col_lock_total_admits,
            "total_releases": self._col_lock_total_releases,
        }
...
class HBMCommandScheduler:
    ...
    def __init__(self,
                 ...
                 age_priority: bool = True,
                 # ---- Col Lock Window (跨 CAM 统一窗口, 跟踪活跃 burst) ----
                 #: False 时 Col Lock Window 完全禁用, 行为等同 v20.1 (默认).
                 col_lock_window_enable: bool = False,
                 #: 窗口容量 (最多同时锁定的 burst 数); enable=True 时生效, 必须 >= 1.
                 col_lock_window_size: int = 8):
        ...
        # 注: Col Lock Window 校验统一在 HBMCommandScheduler.__init__() 后段
        #     创建 ColScheduler 后再 enable 时执行 (避免分散校验逻辑).
        ...
        self._config = SimulationConfig(
            ...
            col_lock_window_enable=col_lock_window_enable,
            col_lock_window_size=col_lock_window_size,
        ...
        # ---- Col Lock Window 启用 (HBMCommandScheduler 是唯一配置入口) ----
        # 默认 disable (行为等同 v20.1). 用户传 enable=True 才启用.
        # 校验统一在此 (避免在 ColScheduler 内重复默认值).
        if col_lock_window_enable:
            if col_lock_window_size < 1:
                raise ValueError(
                    f"col_lock_window_size 必须 >= 1 (enable=True), 当前 {col_lock_window_size}")
            self._col_scheduler.enable_col_lock_window(col_lock_window_size)
    ...
    def _validate_inputs(read_ratio, read_cam_depth, write_cam_depth,
        ...
        # Col Lock Window 校验: enable=True 时 size 必须 >= 1 (ColScheduler 也会校验)
        # 注: ColScheduler.__init__ 已做此校验, 这里仅在 HBMCommandScheduler 入口早期抛错,
        #     避免等到 ColScheduler 实例化时才报.
    ...
    def get_bank_open_duration_stats(self) -> dict:
        ...
        durations: List[int] = []
        for bank in self._banks:
            durations.extend(bank._open_durations)
        if not durations:
    ...
    def _build_summary_lines(self, total_cmds: int, current_cycle: int,
        ...
        return [
            ...
            f"    COL_LOCK_WINDOW: enable={cfg.col_lock_window_enable}, "
            f"size={cfg.col_lock_window_size}",
            ...
            *_format_age_priority_stats(self._row_scheduler.get_age_priority_stats()),
            f"  [Col Lock Window 统计 (v21+, 跨 CAM 统一跟踪活跃 burst)]",
            *_format_col_lock_stats(self._col_scheduler.get_col_lock_stats()),
